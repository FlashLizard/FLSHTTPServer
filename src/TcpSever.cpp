...
void TcpSocket::setSendData(char *sendData, int sentDataLen)
{
	// 此处的sendData一定要改为动态分配的内存，因为在run函数中会delete掉这块内存
	char *data = new char[sentDataLen];
	memcpy((void *)data, (void *)sendData, sentDataLen);
	this->sendDataQueue.push(std::make_pair(data, sentDataLen));
...
int TcpSocket::run()
{
	...
	*/
	...
	{ // FIONBIO：允许或禁止套接口s的非阻塞模式。
		Log::error("ioctlsocket() failed with error!\n");
		return 5;
	}
	Log::info("ioctlsocket() for server socket ok!	Waiting for client connection and data\n");
	...
	while (true)
	{
		if (this->shouldClose)
		{
			...
			{
				status = closesocket(sessionSocket);
				if (status == SOCKET_ERROR)
				{
					Log::error("Tcp close fail");
					return 2;
				}
			}
			status = closesocket(srvSocket);
			if (status == SOCKET_ERROR)
			{
				Log::error("Tcp close fail");
				return 2;
		...
		*/
		...
		if (FD_ISSET(srvSocket, &rfds))
		{
			...
			if (sessionSocket != INVALID_SOCKET)
				Log::info("Socket listen one client request!\n");
			...
			Log::info("ioctlsocket() for session socket ok!	Waiting for client connection and data\n");
		...
		if (nTotal > 0)
		{
			...
			if (FD_ISSET(sessionSocket, &rfds))
			{
				...
				if (rtn > 0)
				{
					Log::debug("Received %d bytes from client: %s\n", rtn, recvBuf);
					TcpCallback callback = this->genTcpCallback();
					callback(*this, recvBuf, rtn);
					Log::debug("callback");
				}
				else
		...
		if (nTotal > 0) //收到消息时，可能处于不可写的状态
		{
			if (FD_ISSET(sessionSocket, &wfds))
			{
				nTotal--;
				while (!this->sendDataQueue.empty())
				{
					auto data = this->sendDataQueue.front();
					rtn = send(sessionSocket, data.first, data.second, 0);
					printf("%s %d\n",data.first,data.second);
					while (rtn != data.second)
					{
						rtn = send(sessionSocket, data.first, data.second, 0);
					}
					delete data.first;
